...
class VersioningTests(unittest.TestCase):
    ...
    def test_project_version_and_development_status_are_consistent(self) -> None:
        ...
        self.assertRegex(claude, r"(?m)^version:\s*0\.4\.2\s*$")
        self.assertRegex(state, r"(?m)^version:\s*0\.4\.2\s*$")
        self.assertIn("0.4.2", changelog)
    ...
    def test_readme_and_badges_parity(self) -> None:
        ...
        for readme in (readme_en, readme_de):
            self.assertRegex(readme, r"Pytest-\d+%20passed")
    ...
    def test_llms_txt_and_manifest_parity(self) -> None:
        ...
        self.assertIn("Last-checked: 2026-09-12", llms_txt)
